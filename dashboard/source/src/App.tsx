...
import ResourceActivityCharts, { RESOURCE_HISTORY_WINDOW_MS, type ResourceHistoryRange, type ResourceSpikeSelection } from "./components/ResourceActivityCharts";
import { AISignalContent, AskOpsPilotModal, RemediationDialog, healthyAISignal, type AISignal, type RemediationAction } from "./components/OpsPilotIntelligence";
...
const commandGroups: Record<string, [string, string][]> = {
  ...
  Logs: [
    ...
    ["journalctl -p 3 -xb -n 50 --no-pager", "Current-boot priority-3 errors for RCA"],
...
function FluentChart({ samples, range, loading = false, onSpikeSelect }: { samples: MetricSample[]; range: HistoryRange; loading?: boolean; onSpikeSelect?: (selection: ResourceSpikeSelection) => void }) {
  return <ResourceActivityCharts samples={samples} range={range} loading={loading} onSpikeSelect={onSpikeSelect} />;
}
...
function OverviewView({ samples, historySamples, historyRange, setHistoryRange, historyLoading, setView, openDetail, openJira, openAI, openRemediation, aiSignal, nodeState, setNodeState }: { samples: MetricSample[]; historySamples: MetricSample[]; historyRange: HistoryRange; setHistoryRange: (range: HistoryRange) => void; historyLoading: boolean; setView: (view: FluentView) => void; openDetail: (metric: DetailMetric) => void; openJira: (metric: string) => void; openAI: (question?: string) => void; openRemediation: (action: RemediationAction) => void; aiSignal: AISignal; nodeState: NodeState; setNodeState: (state: NodeState) => void }) {
  const current = samples.at(-1)!;
  ...
  return <div className="f-view">
    ...
    <section className="f-dashboard-grid">
      <FluentPanel eyebrow="LIVE TELEMETRY" title="Resource activity" className="f-wide" action={<RangeSelector value={historyRange} onChange={setHistoryRange} loading={historyLoading} />}><FluentChart samples={historySamples} range={historyRange} loading={historyLoading} onSpikeSelect={selection => openAI(`Investigate the ${selection.title} value ${selection.value} at ${new Date(selection.timestamp).toISOString()}. Prove the root cause from that timeframe.`)} /></FluentPanel>
      <FluentPanel eyebrow="AI SIGNAL" title="What needs your attention" action={<button className="f-text-action" onClick={() => openAI()}>Investigate with AI →</button>}><AISignalContent signal={aiSignal} companion={<PolarBear size="large" />} onAsk={openAI} onRemediate={openRemediation} /></FluentPanel>
...
function AIWorkspace({ run, samples, openAI, signal }: { run: (command: CommandSpec) => void; samples: MetricSample[]; openAI: (question?: string) => void; signal: AISignal }) {
  const current = samples.at(-1)!;
  const evidence = commands.filter(item => ["top -b -n 1","df -hT","ss -s","journalctl -p 3 -xb -n 50 --no-pager","systemctl --failed --no-pager"].includes(item.command));
  return <div className="f-view"><header className="f-page-title"><div><small>AUTONOMOUS AI OPERATIONS</small><h1>Evidence first. Cause before action.</h1><p>OpsPilot correlates metrics, process state, journals, filesystems, sockets, and services into strict structured RCA without becoming an uncontrolled remote shell.</p></div><FluentStatus>JSON contract enforced</FluentStatus></header>
    <section className="f-ai-workspace"><div className="f-ai-conversation"><header><PolarBear size="large" mood="watching" /><div><b>OpsPilot Investigator</b><small>{signal.provider?.configured ? `${signal.provider.model} · Responses API` : "Deterministic evidence engine · no external model configured"}</small></div><FluentStatus tone={signal.status === "critical" ? "bad" : signal.status === "warning" ? "warn" : "good"}>{signal.status}</FluentStatus></header><div className="f-ai-messages"><article><PolarBear size="small" mood="watching" /><p>{`Current live evidence: CPU ${current.cpu}%, memory ${current.memory}%, root filesystem ${current.disk}%, and 1-minute load ${current.load.toFixed(2)}. ${signal.headline}`}</p></article></div><div className="f-ai-prompt"><div><small>Interactive RCA collects approved commands, validates structured JSON, and keeps every state change behind exact-command confirmation.</small></div><Button appearance="primary" icon={<Bot24Regular />} onClick={() => openAI()}>Open Ask OpsPilot AI</Button></div></div>
      <aside><FluentPanel eyebrow="AUTONOMOUS SIGNAL" title="Latest assessment"><div className="f-evidence-list"><span><i className={signal.status === "healthy" ? "good" : "warn"} /><b>{signal.headline}</b><em>{signal.summary}</em></span>{signal.predictive_warnings.map(item => <span key={item.metric}><i className="warn" /><b>{item.metric} forecast</b><em>{item.hours_to_exhaustion}h to projected exhaustion</em></span>)}</div></FluentPanel><FluentPanel eyebrow="EVIDENCE PLAN" title="RCA core checks"><div className="f-evidence-plan">{evidence.map((item, index) => <button key={item.id} onClick={() => run(item)}><span>{String(index + 1).padStart(2,"0")}</span><div><code>{item.command}</code><small>{item.description}</small></div><b>Run →</b></button>)}</div></FluentPanel><FluentPanel eyebrow="GUARDRAILS" title="Human control remains absolute"><ul className="f-guardrails"><li>✓ Evidence excerpts must exist in raw output</li><li>✓ Natural language maps only to fixed diagnostics</li><li>✓ Forecasting runs locally without an LLM</li><li>× No arbitrary shell or model-generated execution</li><li>× No privileged remediation inside the web sidecar</li></ul></FluentPanel></aside>
    </section>
  </div>;
}
...
export default function FluentOpsPilot() {
  ...
  const [aiSignal, setAiSignal] = useState<AISignal>(() => healthyAISignal());
    ...
    const collect = async () => {
      try {
        ...
        const next: MetricSample = {
          timestamp: Date.parse(String(data.generated_at)) || Date.now(),
        ...
        if (data.ai_signal && typeof data.ai_signal === "object") setAiSignal(data.ai_signal as AISignal);
  ...
  return <FluentProvider theme={theme === "light" ? webLightTheme : webDarkTheme} className="f-provider"><main className={`f-shell ${theme} state-${nodeState}`}>
    ...
    <AskOpsPilotModal open={aiOpen} close={() => setAiOpen(false)} signal={aiSignal} sample={samples.at(-1)!} companion={<PolarBear size="large" mood="watching" />} initialQuestion={aiQuestion} clearInitialQuestion={() => setAiQuestion("")} />
    <RemediationDialog action={remediation} close={() => setRemediation(null)} />
